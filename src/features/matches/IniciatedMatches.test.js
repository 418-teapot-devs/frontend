...
test("Renders all matches' creators", async () => {
    const {getAllByTestId} = render(<IniciatedMatches matches={iniciated}/>)
    const matches_creators = getAllByTestId('match-username').map((cell) => cell.textContent)
...
test("Renders all matches' games", async () => {
    const {getAllByTestId} = render(<IniciatedMatches matches={iniciated}/>)
    const matches_games = getAllByTestId('match-games').map((cell) => cell.textContent)
...
test("Renders all matches' rounds", async () => {
    const {getAllByTestId} = render(<IniciatedMatches matches={iniciated}/>)
    const matches_rounds = getAllByTestId('match-rounds').map((cell) => cell.textContent)
...
test("Renders results when there are ready", async () => {
    const {getAllByTestId} = render(<IniciatedMatches matches={iniciated}/>)
    const matches_rank = getAllByTestId('match-rank').map((cell) => cell.textContent)
    const iniciated_rank = iniciated.map((match) => {
      if (match.ranking_position == null) { return "" }
      else { return String(match.ranking_position) }
    })
    expect(matches_rank).toEqual(iniciated_rank)

    const matches_mmrwon = getAllByTestId('match-mmrwon').map((cell) => cell.textContent)
    const iniciated_mmrwon = iniciated.map((match) => {
      if (match.MMR_won == null) { return "" }
      else { return "+" + String(match.MMR_won) }
    })
...
test("Renders all robots", async () => {
  ...
  const expected_robots = iniciated.reduce((a, match) => a + match.robots.length, 0);
...
test("Renders locks if match is private", async () => {
  ...
  const expected_isprivate = iniciated.reduce((a, match) => a + (match.is_private? 1: 0), 0);
