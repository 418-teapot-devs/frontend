...
test("should upload", async () => {
  ...
  await user.upload(screen.getByLabelText("code"), new File(["hola"], "robot.py", { type: ".py" }))

  await user.click(screen.getByRole("button", { name: /Crear/i }))

  expect(await screen.findByText(/Se subió el robot con éxito/i)).toBeInTheDocument()
});
...
test("should not upload", async () => {
  ...
  expect(await screen.findByText(/Ya cuentas con un robot con ese nombre/i)).toBeInTheDocument()
})
...
test("no code, should not upload", async () => {
  ...
  expect(await screen.findByText(/El código de su robot es requerido/i)).toBeInTheDocument()
...
test("unknown error, should not upload", async () => {
  ...
  expect(await screen.findByText(/No se pudo subir el robot/i)).toBeInTheDocument()
})
